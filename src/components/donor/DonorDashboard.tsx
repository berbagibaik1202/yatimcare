...
export const DonorDashboard: React.FC<DonorDashboardProps> = ({
  ...
  const myDonations = currentUser
    ? allDonations.filter(d => d.donorEmail === currentUser.email)
    : [];
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 text-center shadow-sm">
          <Award className="mx-auto h-12 w-12 text-emerald-600" />
          <h2 className="mt-4 text-2xl font-black text-slate-900">Dashboard donatur membutuhkan login</h2>
          <p className="mt-2 text-sm text-slate-600">
            Masuk menggunakan akun donatur yang terdaftar untuk melihat riwayat donasi pribadi.
          </p>
        </div>
      </div>
    );
  ...
  const handleExportPDF = () => {
    ...
    const summary = [
      { label: 'Nama Donatur', value: currentUser.name },
      ...
      <div className="bg-gradient-to-r from-emerald-800 via-emerald-700 to-teal-800 text-white rounded-3xl p-8 shadow-md border border-emerald-600 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div className="space-y-2 relative z-10">
          ...
          <h1 className="font-serif font-black text-2xl sm:text-3xl text-white">
            Terima Kasih, Ibu {currentUser.name}
          </h1>
