...
export const GuardianPortal: React.FC<GuardianPortalProps> = ({ onRefreshData }) => {
  ...
  const guardianChildren = currentUser
    ? allChildren.filter(c => c.guardianPhone === currentUser.phone)
    : [];
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="rounded-[32px] border border-slate-200 bg-white p-8 text-center shadow-sm">
          ...
          <h2 className="mt-4 text-2xl font-black text-slate-900">Portal Wali membutuhkan login</h2>
          <p className="mt-2 text-sm text-slate-600">
            Masuk menggunakan akun wali yang terdaftar untuk melihat data anak dan mengajukan pendaftaran.
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-gradient-to-r from-emerald-800 via-emerald-700 to-teal-800 text-white rounded-3xl p-8 shadow-md border border-emerald-600 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div className="space-y-2 relative z-10">
          ...
          <h1 className="font-serif font-black text-2xl sm:text-3xl text-white">
            Selamat Datang, Ibu {currentUser.name}
          </h1>
